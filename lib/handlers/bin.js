...
module.exports = Observable.extend({
  constructor: function BinHandler(sandbox) {
    ...
    var methods = Object.getOwnPropertyNames(BinHandler.prototype).filter(
      function(prop) {
  ...
  getFromPost: function(req, res, next) {
    var processorTypes = 'jade markdown less stylus sass scss coffeescript processing traceur typescript jsx babel'.split(
      ...
      allTypes = 'html css javascript'
        .split(' ')
        .concat(processorTypes)
  ...
  apiGetBin: function(req, res, next) {
    this.applyProcessors(req.bin).then(
      function(bin) {
  ...
  testPreviewAllowed: function(req, res, next) {
    ...
    if (
      (config.security.allowAnonymousPreview !== true &&
        user === 'anonymous') ||
      !user
    ) {
      ...
      if ((Date.now() - created.getTime()) / 1000 / 60 > 90) {
        var msg =
          'This bin was created anonymously and its free preview time has expired (<a target="_blank" href="http://jsbin.com/blog/security-limited-output#limitedfulloutputforanonymousbins">learn why</a>).';

        if (!req.session.user) {
          msg += ' — <a href="/register">Get a free unrestricted account</a>';
        } else {
          msg +=
            ' — <a class="clone" href="/clone">Clone it to enable the full preview</a>';
        }
        ...
        new Promise(function(resolve, reject) {
          ...
          if (req.subdomain) {
            ...
            if (custom[req.subdomain]) {
              ...
              req.subdomain.toLowerCase() !==
              (undefsafe(bin, 'metadata.name') || '').toLowerCase()
            ) {
              ...
              this.formatPreview(
                ...
                function(err, formatted) {
                  ...
                  if (req.bin.settings) {
                    ...
                    if (
                      req.bin.settings.headers &&
                      req.bin.settings.headers.constructor === Object
                    ) {
                      try {
          ...
          if (settings.processors) {
            ...
            if (reverseProcessorLookup[format]) {
              ...
              if (processors.support(settings.processors[format])) {
                ...
                _this.applyProcessors(bin).then(
                  function(results) {
                    bin = results[0];
  ...
  getUserBins: function(req, res, next) {
    ...
    var acceptsJSON =
      (req.header('Accept', '') || req.header('content-type') || '').indexOf(
        'application/json'
      ) > -1;
    ...
    this.models.user.load(
      ...
      function(error, user) {
        ...
        this.models.user[dbMethod](
          req.params.user || req.session.user.name,
          function(err, bins) {
            if (err) {
              return next(404);
            }
  ...
  getLatestForUser: function(req, res, next) {
    ...
    this.models.user.getLatestBin(
      req.user.name,
      req.params.n || 0,
      undefsafe(req, 'session.user.name'),
      function(err, bin) {
        if (err && err !== 401) {
          return next(err);
        }
        ...
        if (!bin || err === 401) {
          ...
          return next(
            new errors.BinNotFound('Could not find bin: ' + req.params.bin)
          );
        } else {
          req.bin = bin;
          next();
  ...
  setBinAsPrivate: function(req, res, next) {
    ...
    this.models.bin.setBinVisibility(
      req.bin,
  ...
  createRevision: function(req, res, next) {
    ...
    if (req.param('method') === 'save') {
      ...
      params.visibility = req.bin.metadata
        ? req.bin.metadata.visibility
        : 'public';
      ...
      this.validateBin(params, function(err) {
        ...
        handler.models.user.isOwnerOf(username, params, function(err, result) {
          ...
          if (result.found && result.result.active === 'n') {
            // this is a deleted bin
            metrics.increment('bin.fork');
            delete params.revision;
          } else if (
            result.isowner ||
            req.bin.streaming_key === req.body.checksum
          ) {
            ...
            if (result.isowner) {
              ...
              handler.models.bin.updateBinData(
                req.bin,
                { streaming_key: '' },
                function(error) {
                  // jshint ignore:line
                  if (error) {
                    console.error(
                      'models.bin.updateBinData:streaming_key',
                      error
                    );
                  }
                }
      ...
      this.validateBin(bin, function(err) {
        ...
        handler.models.bin.updatePanel(panel, params, function(err, result) {
          ...
          if (req.session.user) {
            handler.models.user.updateOwners(
              req.session.user.name,
              params,
              function() {}
            );
      ...
      if (!result && err === 401) {
        ...
        if (!result) {
          req.bin = {
            url: req.params.bin,
            revision: 0,
            streaming_key: req.body.checksum,
          };
  ...
  validateBin: function(bin, fn) {
    ...
    var panels = 'html javascript css'
      .split(' ')
      .map(function(key) {
        return (bin[key] || '').trim();
      })
    ...
    if (!blacklist.validate(bin) && !undefsafe(bin, 'metadata.verified')) {
      ...
      fn(
        new errors.BadRequest(
          'Unable to save: Post contains blacklisted content'
        )
      );
    ...
    this.protectVisibility(
      ...
      function(err, bin) {
        var template = this.templateFromBin(bin),
          ...
          user = req.session.user || {},
          production =
            req.cookies && req.cookies.debug ? false : helpers.production,
          jsbin;
        ...
        jsbin = this.jsbin(bin.url ? bin : {}, {
          ...
          checksum:
            created.url === bin.url &&
            created.revision === bin.revision &&
            created.checksum,
        });

        // only expose the parts of the user we want
        var userfields =
          'avatar name bincount created pro settings github_token';
        jsbin.user = _.pick.apply(_, [user].concat(userfields.split(' ')));

        if (!jsbin.user.avatar && req.session.user) {
          req.session.user.avatar = jsbin.user.avatar = req.app.locals.gravatar(
            req.session.user
          );
        }

        if (jsbin.user.avatar) {
          jsbin.user.large_avatar = req.app.locals.gravatar(
            req.session.user,
            120
          ); // jshint ignore:line
        }
        ...
        if (
          !jsbin.state.checksum &&
          jsbin.state.streaming === true &&
        ...
        if (jsbin.user) {
          // i.e. logged in and not embed views
  ...
  render: function renderEdit(req, res, bin) {
    ...
    res.render('index', {
      ...
      analyticsAltId:
        req.embed && undefsafe(config, 'analytics.render-id')
          ? undefsafe(config, 'analytics.render-id')
          : null,
      bin: req.bin || null,
      ...
      code_id_path: url,
      concat:
        req.cookies && req.cookies.debug
          ? req.cookies.debug === 'concat'
          : false,
      ...
      url: url,
      user: user || null,
      vanity: features('vanity', req)
        ? 'http://' + req.session.user.name + '.' + req.app.get('url host')
        : root,
  ...
  renderCreated: function(req, res, bin) {
    ...
    if (req.ajax) {
      if (req.param('format', '').toLowerCase() === 'plain') {
        return res
          .contentType('txt')
          .send(this.helpers.editUrlForBin(bin, true));
      }
  ...
  jsbin: function(bin, options) {
    ...
    var panels =
      binSettings.panels ||
      Object.keys(_.pick(bin, 'html', 'javascript', 'css')).filter(function(
        panel
      ) {
    ...
    options.metadata = _.pick.apply(
      _,
      [options.metadata].concat(
        'archive avatar created last_login name pro summary updated visibility'.split(
          ' '
        )
      )
    );
    ...
    var result = {
      ...
      settings: options.settings
        ? _.extend({}, { panels: panels }, options.settings)
        : { panels: panels },
    };
  ...
  loadFiles: function(files, fn) {
    ...
    async
      .files(files, this.helpers.set('views'))
      .readFile('utf8')
  ...
  applyProcessors: function(bin) {
    ...
    if (
      ...
      Object.keys(binSettings.processors).length
    ) {
      return Promise.all(
        Object.keys(binSettings.processors).map(function(panel) {
  ...
  formatPreview: function(req, bin, options, fn) {
    ...
    function onAnalyticsComplete(err, formatted, analytics) {
      ...
      if (options.edit && !req.ajax) {
        var data = {
          static: helpers.urlForStatic('', req.secure),
          root: helpers.url('/', true, req.secure),
        };
        insert.push(
          '<script src="' +
            helpers.urlForStatic(
              'js/render/edit.js?' + helpers.set('version'),
              req.secure
            ) +
            '"></script>'
        );
        insert.push(
          '<script>jsbinShowEdit && jsbinShowEdit(' +
            JSON.stringify(data) +
            ');</script>'
        );
      }
      ...
      if (!options.silent && _this.models.bin.isStreaming(bin)) {
        // jshint ignore:line
        _this.emit('render-scripts', scripts);
        insert = insert.concat(
          scripts.map(function(script) {
            script =
              script.indexOf('http') === 0
                ? script
                : helpers.urlForStatic(script, req.secure);
  ...
  delete: function(req, res, next) {
    ...
    this.models.bin.updateOwnersData(
      req.bin,
      { archive: -1, url: 'deleted/' + req.bin.url },
      function() {}
    );

    this.models.bin.updateBinData(
      req.bin,
      { active: 'n', url: 'deleted/' + req.bin.url },
      function(error) {
        if (error) {
          console.error(error);
          return res.send(400, {
            error: 'The bin could not be deleted.',
            detail: error,
          });
        }
  ...
  transfer: function(req, res, next) {
    ...
    self.models.user.load(req.body.to, function(error, user) {
      ...
      self.models.bin.updateOwnersData(req.bin, { name: req.body.to }, function(
        error,
        result
      ) {
        if (error) {
  ...
  ensureOwnership: function(req, res, next) {
    // signed in owner
    if (
      undefsafe(req, 'bin.metadata.name') ===
      undefsafe(req, 'session.user.name')
    ) {
      return next();
    }
    ...
    // anonymous owner
    if (
      undefsafe(req, 'bin.streaming_key') === undefsafe(req, 'body.checksum')
    ) {
      return next();
    }
  ...
  updateSettings: function(req, res, next) {
    ...
    var updatedSettings = JSON.stringify(
      deepExtend(req.bin.settings, body, force)
    );
    this.models.bin.updateBinData(
      req.bin,
